...
import { getAuthApiRequestBase, getInternalNextOrigin } from "@/lib/env";
...
/** Resolves login POST URL (XHR bypasses Axios; still needs absolute URL when base is relative). */
function resolveLoginRequestUrl(path: string): string {
...
export async function loginRequest(body: LoginPayload): Promise<LoginResult> {
  ...
  const url = resolveLoginRequestUrl(apiRoutes.auth.login());
